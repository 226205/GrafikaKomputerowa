...
function gl_initBuffers () 
{
	...
	[
		1, 1, 1,      // wierzchołek #0
		1, 0, 0,       // kolor: czerwony  model RGB
		-1, -1, 1 ,     // wierzchołek #1
	...
	var triangleFaces = // macierz mowiaca ktore wierzcholki polaczyc ze soba aby stworzyc trojkat
	[
		0,1,2,
		0,2,3,
		1,2,3,
  ...
  window.cancelAnimationFrame(animation)
   ...
   var animate = function (time) {
	...
	var dAngle = rotationSpeed * (time - timeOld);
      ...
      gl_ctx.vertexAttribPointer(_position, 3, gl_ctx.FLOAT, false, 4*(3+3), 0); // obiekt 3d wiec 3 wspolrzedne ma kazdy wierzcholek
      gl_ctx.vertexAttribPointer(_color, 3, gl_ctx.FLOAT, false, 4*(3+3), 3*4);
      gl_ctx.bindBuffer(gl_ctx.ARRAY_BUFFER, _triangleVertexBuffer);
      gl_ctx.bindBuffer(gl_ctx.ELEMENT_ARRAY_BUFFER, _triangleFacesBuffer);
      gl_ctx.drawElements(gl_ctx.TRIANGLES, 12, gl_ctx.UNSIGNED_SHORT, 0); //kazdy nowy element jest renderowany przez 3 (trojkat) kolejne wartosci, a jako ze mamy 4 sciany to wstawiamy 12
      gl_ctx.flush();
